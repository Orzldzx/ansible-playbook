...
if [[ $1 -eq "3306" ]]; then
    ...
    MYSQL_SOCK=$(cat /etc/my${1}.cnf |grep -wi sock |sort |uniq -c |awk '{print $NF}' | uniq)
fi
MYSQL_PWD=""

case $2 in
    Version)
        result=$(mysqladmin -uroot -p${MYSQL_PWD} -S $MYSQL_SOCK version 2>/dev/null |awk '$1 ~ /^Server/ {print $3}')
        echo $result
        ;;
    Ping)
